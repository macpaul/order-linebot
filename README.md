...
- 📊 **Google 試算表即時資料庫 & 管理員自訂介面**：
  ...
  - `Children` 名冊完整管理每位同仁名下登記之小孩（`ChildName`、備註 `Note`），支援 LINE 聊天室指令與試算表雙向同步。
  - 管理員可直接在試算表中調整每天店家、菜單與截止時間，即時讀取生效。
  - Google 試算表整合 **「🍱 便當訂餐管理」** 自訂選單，方便一鍵初始化結構、匯入外送菜單、匯入自訂餐廳菜單與重算統計。
- 👦 **多小孩（大寶/二寶）分餐管理與按鈕直選體驗**：
  - **單一小孩智慧預設**：同仁名下若僅登記 1 位小孩，點餐時系統自動預設分配給該小孩，免去重複確認流程，一秒極速完成點餐！
  - **多小孩直選快捷按鈕**：同仁若登記 2 位或以上小孩，點餐時 LINE 畫面底部自動彈出 Quick Reply 浮動按鈕（`[👦 大寶]`、`[👦 二寶]`、`[👤 本人]`、`[✏️ 手動輸入小孩名字]`），兩指輕點直選完成分餐，免手動打字！
  - **未登記小孩引導快捷按鈕**：同仁若尚未登記小孩，點餐時亦會自動彈出快捷按鈕（`[✏️ 手動輸入小孩名字]`、`[👤 本人]`、`[👶 小孩選單]`），點擊即可手動輸入小孩姓名並自動同步建檔，或一鍵確認為本人用餐。
  ...
  - **幫助選單與直覺子選單**：「`幫助`」卡片整合「`👶 小孩選單`」按鈕，點擊即自動跳出小孩管理子選單卡片（查看名單、批次登記、新增小孩、刪除小孩）；其中「設定小孩」點擊後自動跳出引導提示語與輸入框預填快捷按鈕（引導使用逗號 `,` 分隔多位小孩），「新增小孩」點擊後自動跳出引導提示語與輸入框預填快捷按鈕（引導依範例輸入單一小孩姓名與備註），「刪除小孩」點擊後自動跳出名冊快捷按鈕（Quick Reply）與「取消刪除」按鈕，一鍵直覺操作免手動打字。
- 📄 **超長菜單自動分頁與品項點餐完全隔離防護**：
  - 為符合 LINE Flex Message 限制（Box 子元件 100 筆上限、JSON Payload 30KB 限制），品項超過長度限制時（預設每頁 20 項），自動啟用分頁顯示。
  - 卡片標題與備註處清晰標註目前頁數與總頁數（例如：`📄 第 1 / 3 頁 (共 3 頁)`、`全店共 45 道餐點`）。
  - 菜單底部提供各分頁導航按鈕（例如：`[ 📄 第 2 頁 ]`、`[ 📄 第 3 頁 ]`），輕觸即可無縫翻頁。
  - **嚴格品項點餐免疫保護**：所有分頁指令（`第2頁`、`菜單 第2頁`、`週一菜單 第2頁`、`page 2` 等）均受嚴格防護，絕不會被誤判為點餐品項寫入訂單試算表。
- 🍱 **彈性點餐語法 & 互動 Flex Message**：
  ...
  - **多元結單模式與支付方式（開單人專屬權限）**：僅限開單人（`ORGANIZER_ID`）有權發送「結單」或點擊卡片結單按鈕，其餘成員操作將被系統攔截並提示權限不足，防止誤觸關閉點餐。支援依設定截止全週或當日訂單（`CLOSE_ORDER_SCOPE`），結單卡片整合 **LINE Pay 轉帳**（支援商家連結或個人 LINE 錢包好友轉帳與好友暱稱指引）、**銀行跨行匯款帳號** 與 **收款 QR Code 圖片**（支援 Google Drive 分享連結自動轉換，點擊可放大掃碼）。
...
| 指令 | 說明 | 範例 |
| :--- | :--- | :--- |
| `菜單` / `菜單 第X頁` / `第X頁` | 查看今日配合店家菜單（品項過多自動分頁，可指定頁數或點擊翻頁） | `菜單`、`第2頁`、`菜單 第2頁` |
| `本週菜單` | 顯示週一至週五每日配合店家與排程卡片 | `本週菜單` |
| `週X菜單` / `週X菜單 第X頁` | 查看指定星期的店家專屬菜單卡片（支援分頁） | `週一菜單`、`週一菜單 第2頁` |
| `週X+1 [餐點]` | 跨日/梯次點餐（單則訊息可預約整週多日餐點） | `週一 排骨飯+1, 週二 炸雞腿+1, 週四 控肉飯+1` |
| `+1 [餐點名稱]` | 當日快速點餐（預設 1 份；單一小孩自動預設、多小孩或未登記時彈出 Quick Reply 快捷按鈕） | `+1 招牌排骨飯` |
...
| `小孩選單` / `小孩管理` | 開啟小孩與用餐對象管理選單卡片（幫助卡片亦有專屬快捷按鈕） | `小孩選單`、`小孩管理` |
| `設定小孩` | 顯示批次設定引導提示語與鍵盤預填快捷按鈕（提示使用逗號「,」分隔多位小孩） | `設定小孩` |
| `我的小孩` / `小孩名單` | 查詢已登記的小孩名冊與快速點餐指引（圖文卡片） | `我的小孩`、`小孩名單` |
| `設定小孩 [小孩1], [小孩2]` | 批次綁定登記個人名下的小孩名冊（使用逗號「,」分隔） | `設定小孩 大寶, 二寶` |
| `新增小孩` / `新增小孩 [姓名] [備註]` | 新增單一小孩姓名與備註（輸入「新增小孩」自動彈出引導提示與鍵盤預填快捷按鈕） | `新增小孩`、`新增小孩 小寶 備註` |
| `刪除小孩` / `刪除小孩 [姓名]` | 移除名下指定的小孩紀錄（輸入「刪除小孩」自動彈出名單快捷按鈕與「取消刪除」） | `刪除小孩`、`刪除小孩 大寶`、`取消刪除` |
...
| `匯入菜單 [週X] [網址]` / `foodpanda匯入` / `ubereats匯入` / `nidin匯入` / `你訂匯入` | 透過 Uber Eats、foodpanda 或你訂 (Nidin) 店家網址/ID 自動匯入菜單 | `匯入菜單 週一 https://www.ubereats.com/...`、`foodpanda匯入 週四 https://www.foodpanda.com.tw/...`、`你訂匯入 週五 https://order.nidin.shop/menu/29638` |
...
2. **多語系使用者自訂模式 (開關式)**：
   ...
   - 幫助卡片將自動浮現「`🌐 設定語言 / Language`」按鈕，成員亦可隨時發送「`設定語言`」或「`lang`」呼叫 7 國語言切換卡片。
   - 系統將每個人的語言偏好存入試算表 `UserPreferences` 頁籤，往後的個人收據、訂單查詢與幫助選單即以該成員的母語呈現！
   - 所有指令支援母語別名（如英文發送 `help`、日文 `ヘルプ`、韓文 `도움말`、泰文 `ช่วยเหลือ`、印尼文 `bantuan`、越南文 `trợ giúp` 均可正常回應）。
...
│   ├── NidinService.js            # 你訂 (Nidin) 店家菜單抓取、價格轉換與正規化模組
...
├── scripts/
│   └── bundle.js                  # 自動將 10 個 src/ 模組打包至 dist/Code.gs 的腳本
└── tests/
    └── test_order_flow.js         # 包含多語系、週梯次點餐、Uber Eats、foodpanda 與你訂 (Nidin) 匯入的端到端完整測試套件
```
...
```bash
# 執行包含週梯次、Uber Eats、foodpanda 與你訂 (Nidin) 匯入的端到端測試套件
npm test
