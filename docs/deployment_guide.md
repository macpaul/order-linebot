...
| `DEFAULT_LOCALE` | `zh-TW` | **全域系統預設語系**：支援 `zh-TW`、`en`、`ja`、`ko`、`th`、`id`、`vi`。 |
| `ENABLE_USER_LOCALE` | `false` | **是否允許使用者透過選單自訂語言**：`true` 允許並在幫助選單顯示語言按鈕 / `false` 統一使用預設語系。 |
| `ALLOW_WEEKEND_ORDERING` | `false` | **是否支援週末點餐 (週六與週日)**：<br>• `false`（預設）：僅支援週一至週五平日點餐。週六與週日為預約準備日，若開單人開啟點餐 (`IS_ORDERING_OPEN: true`)，成員可自由預訂下週梯次。<br>• `true`：全面支援週末點餐，支援週六與週日菜單、排程、點餐、結單與統計匯總。 |
...
#### 1. LINE 按鈕點餐體驗（智慧單一小孩預設 + Quick Reply 浮動按鈕 + openKeyboard 方案）
- **單一小孩智慧預設**：若同仁僅登記 1 位小孩，點擊菜單上的「+1 點餐」按鈕或輸入 `+1 餐點` 時，系統自動預設將餐點分配給該小孩，免去重複確認流程，一鍵直出個人收據！
- **多小孩直選快捷按鈕**：
  1. 若同仁登記 2 位或以上小孩，點擊「+1 點餐」按鈕後，LINE 畫面底部立即浮出 Quick Reply 快速選單：
     - `[👦 大寶]`（點擊直接發送 `+1 招牌便當 (大寶)`）
     - `[👦 二寶]`（點擊直接發送 `+1 招牌便當 (二寶)`）
     - `[👤 本人]`（點擊直接發送 `+1 招牌便當 (本人)`）
     - `[✏️ 手動輸入小孩名字]`（利用 LINE 原生 `openKeyboard` + `fillInText` 自動開啟鍵盤並預填 `+1 招牌便當 ()` 讓游標停在括號中輸入小孩名字）
  2. 家長全程無需手動打字，只需點擊兩下即完成精確分餐！
- **未登記小孩引導快捷按鈕**：
  - 同仁若尚未登記小孩，點擊「+1 點餐」按鈕時，系統亦會自動彈出 Quick Reply 浮動按鈕：
    - `[✏️ 手動輸入小孩名字]`（自動開啟鍵盤預填 `+1 招牌便當 ()`，送出後系統自動將該小孩登記進名冊）
    - `[👤 本人]`（點擊直接下單為本人餐點）
    - `[👶 小孩選單]`（開啟小孩管理子選單進行批次或個別維護）
...
- **LINE 幫助選單整合**：
  - 「`幫助`」卡片內建「`👶 小孩選單`」快捷按鈕，點擊立即開啟小孩專屬圖文子選單。
- **LINE 直覺指令與子選單**：
  - `小孩選單` / `小孩管理`：開啟小孩與用餐對象管理選單卡片（含 4 大直覺操作按鈕）。
...
- **機制**：系統在核對開單人管理權限（如結單、強制取消他人餐點、取消全體預約等敏感動作）時，**完全以 `ORGANIZER_ID` (`userId`) 為唯一依據**，徹底移除對 `ORGANIZER_NAME` 顯示名稱的授權判斷，杜絕群組成員將自身暱稱改為開單人名稱來取得管理員權限之安全隱患。所有結單指令（`結單`、`截止`、`截止訂餐`、`本週結單`、`今日結單`）及統計卡片上的快捷結單按鈕均僅限開單人可執行；非開單人操作將被系統攔截並提示 `⚠️ 權限不足：只有開單人可以執行結單！`，防止群組成員誤觸導致點餐提早關閉。
...
#### 2. 開單人 LINE 私密推播與多開單人維護機制 (`ORGANIZER_PUSH_ID`)
- **推播需求與個資矛盾**：LINE 官方 `pushText` API 強制要求目標必須是真實未雜湊的 LINE User ID (`U...`)。而在 `HASHED_ID` 或 `NICKNAME` 隱私模式下，公開的試算表中絕不留存真實 User ID。
- **統一逗號分隔格式 (支援方案 A、B、C)**：
  開單人可於 Apps Script 專案設定的 **「指令碼屬性 (Script Properties)」** 新增屬性 `ORGANIZER_PUSH_ID`，支援使用逗號 `,` 區隔的多種彈性設定方式（存放於 Google 雲端專案後台，**試算表檢視者完全無法看見**）：

| 應用情境 | `ORGANIZER_PUSH_ID` 設定範例 | 運作機制與說明 |
| :--- | :--- | :--- |
| **方案 A：免設定模式<br>(零維護負擔)** | *(留空不安裝任何屬性)* | • **完全不需後台維護**。<br>• 系統在隱私模式下安全略過推播，開單人直接於群組點擊或輸入「`統計`」查看即時點餐卡片。<br>• 試算表保持 100% 最高隱私等級。 |
| **方案 B-1：純 User ID 逗號列表<br>(自動雜湊辨識開單人)** | `U11111111111111111111111111111111, U22222222222222222222222222222222` | • **多開單人單獨通知**：當某位開單人家長在群組開單，系統會自動比對名單中誰的雜湊值與當前 `ORGANIZER_ID` 相符。<br>• **精準單獨推播**：只有開該次單的開單人會收到加訂/退訂推播，其餘開單人不會被騷擾！ |
| **方案 B-2：鍵值對應逗號列表<br>(暱稱或雜湊直覺指名)** | `小明媽媽:U11111111111111111111111111111111, 小華爸爸:U22222222222222222222222222222222` | • **支援格式**：`暱稱:UID` 或 `usr_雜湊碼:UID`（支援冒號 `:` 或等號 `=`）。<br>• 系統在開單人為「小華爸爸」時，自動指名單獨發送給小華爸爸的 LINE。 |
| **方案 C：全體幹部廣播模式<br>(多位負責人同步接收)** | `*:U99999999999999999999999999999999, 小明媽媽:U11111111111111111111111111111111` | • 使用萬用字元 `*:` 或 `all:`（例如 `*:U999...`），代表該帳號為固定主管/監督人員，無論哪位家長開單，該帳號均會收到加訂/退訂推播。<br>• 若設定為純 User ID 且未比對到特定開單人時，系統亦會安全廣播通知名單內的所有幹部。 |

#### 3. 如何將現有存有明文 LINE User ID 的試算表升級為去識別化雜湊 ID？

若您的 Google 試算表在舊版本時期已累積了許多歷史訂單或小孩名冊，欄位中仍存有未加密的原始 LINE User ID（`U...`），系統提供兩種升級途徑：

##### 途徑一：無痛平滑升級（最推薦 💡 免手動改歷史資料）
1. 複製最新版 [`dist/Code.gs`](dist/Code.gs) 貼入 Apps Script 編輯器儲存。
2. 回到試算表點擊上方選單：**`🍱 便當訂餐管理` ➡️ `📅 檢查/初始化試算表結構`**，系統會自動在 `Config` 中補填 `USER_IDENTIFIER_MODE: 'HASHED_ID'`。
3. **運作效果**：
   - 新產生的點餐訂單與小孩名冊將全面自動以不可逆的 `usr_...` 雜湊代號儲存。
   - 歷史既有訂單無需任何更動，系統查詢與退訂內建**雙軌向下相容比對**，成員依然可正常查單與退訂。

##### 途徑二：一鍵歷史去識別化遷移工具（徹底抹除歷史明文 User ID）
若您希望將試算表中歷史遺留的所有明文 LINE User ID 徹底清除，一鍵轉為雜湊 ID：
1. 部署最新版 [`dist/Code.gs`](dist/Code.gs) 後，重新整理 Google 試算表網頁。
2. 點擊試算表上方自訂選單：**`🍱 便當訂餐管理` ➡️ `🔒 一鍵升級歷史 ID 為去識別化雜湊`**。
3. 系統將會以高效批次寫入（`setValues`）自動執行：
   - 掃描 **`Orders`**：將所有歷史訂單中的原始 LINE User ID（`U...`）轉換為 `usr_xxxxxxxxxxxxxxxx`。
   - 掃描 **`Children`**：將小孩名冊的家長 ID 轉換為 `usr_...`。
   - 掃描 **`Config`**：將現任開單人 `ORGANIZER_ID` 轉換為雜湊 ID，並切換 `USER_IDENTIFIER_MODE` 為 `HASHED_ID`。
   - **冪等性保障**：已是 `usr_` 開頭的雜湊 ID 會自動跳過，絕不重複雜湊。完成後會跳出摘要對話框提示轉換筆數。

---

### Q14：如何設定系統語系與開啟多國籍使用者語言自訂 (i18n)？

本系統支援繁體中文 (`zh-TW`)、英文 (`en`)、日文 (`ja`)、韓文 (`ko`)、泰文 (`th`)、印尼文 (`id`) 與越南文 (`vi`) 等 7 國語言。

#### 1. 全域系統預設語系 (`DEFAULT_LOCALE`)
- **設定位置**：試算表 `Config` 頁籤中的 `DEFAULT_LOCALE`。
- **預設值**：`zh-TW`。
- **可選設定**：`zh-TW`、`en`、`ja`、`ko`、`th`、`id`、`vi`。
- **作用**：當未開啟使用者自訂或使用者尚未設定偏好時，系統所有群組推播、點餐介面與說明卡片均採用此語言。

#### 2. 開放使用者個別切換語言 (`ENABLE_USER_LOCALE`)
- **設定位置**：試算表 `Config` 頁籤中的 `ENABLE_USER_LOCALE`。
- **預設值**：`false`。
- **開關效果**：
  - **`false`（固定單一語系）**：
    - 聊天室「`幫助`」選單僅顯示一般功能按鈕（無語言切換按鈕）。
    - 成員若傳送「`設定語言`」或「`lang`」，機器人會回覆提示管理員目前尚未開放個人切換語言。
  - **`true`（開放使用者自訂）**：
    - 聊天室「`幫助`」選單會自動加入「`🌐 設定語言 / Language`」按鈕。
    - 成員可透過選單或發送「`設定語言`」/「`lang`」呼叫 7 國語言切換卡片。
    - 亦可直接發送語系代碼，如「`設定語言 en`」或「`cài đặt ngôn ngữ vi`」。
    - 系統會將使用者的語言設定存入 `UserPreferences` 頁籤，此後該成員的個人訂單、收據與幫助均會以其自訂的語言顯示。
    - 支援母語指令直接點餐操作（如英文 `help`、日文 `ヘルプ`、韓文 `도움말`、泰文 `ช่วยเหลือ`、印尼文 `bantuan`、越南文 `trợ giúp`）。

---

### Q15：週末點餐與週六預點下週餐點功能說明 (`ALLOW_WEEKEND_ORDERING`)

#### 1. 週六預點下週餐點 (平日點餐模式，預設 `ALLOW_WEEKEND_ORDERING: 'false'`)
- 只要開單人開放點餐 (`IS_ORDERING_OPEN: 'true'`)，成員在週六或週日即可自由預訂下週一至週五的餐點（例如在週六輸入「`+1 招牌排骨飯`」預設登記下週一，或「`週二+1 酥炸雞腿飯`」登記下週二）。
- 過去梯次的星期判定已優化：在週六與週日，下週的週一至週五一律被視為「未來梯次」，不會被誤判為已過期。

#### 2. 開放週末訂餐 (`ALLOW_WEEKEND_ORDERING: 'true'`)
- 若團體或學校週末亦有活動、自習或活動便當需求，可於 `Config` 中將 `ALLOW_WEEKEND_ORDERING` 設為 `true`。
- 開啟後，系統自動將排程與結算週期擴展為 **7 天（週一至週日）**：
  - 支援週六、週日排程、菜單與截單時間。
  - 成員可輸入「`週六+1 [餐點]`」、「`週日+1 [餐點]`」或於週末當日直接「`+1 [餐點]`」。
  - 支援「`週六菜單`」、「`週日菜單`」及各國語言別名（`saturday menu`、`sunday menu`、`土曜メニュー`、`일요일 메뉴` 等）。
  - 「`本週訂單`」與「`本週結單`」會自動彙總週一至週日全 7 天的訂單明細。

---

### Q16：菜單分頁顯示與品項點餐完全隔離防護說明 (LINE Payload 限制防護)

#### 1. 設計背景與限制
- **LINE Flex Message 規範限制**：LINE 官方對單則 Flex Message 設有嚴格限制：
  - 單一 Box 容器內的子元件數量上限為 100 筆。
  - 單一 Flex 訊息的 JSON Payload 大小上限為 30KB。
- 當店家菜單品項較多（例如熱炒、便當多樣配菜或飲料店全品項多達數十道）時，若一次將所有品項塞入單一 Flex Bubble，可能導致訊息超過 30KB 或超過 Box 限制而遭 LINE 平台拒絕發送（400 Bad Request）。

#### 2. 自動分頁機制
- **預設分頁大小**：每頁上限預設為 20 道餐點（`pageSize: 20`），兼顧版面美觀、閱讀舒適度與 Payload 安全餘量（約 10~15KB）。
- **標題與備註頁數提示**：
  - **頂部 Header**：若總頁數大於 1 頁，於截止時間旁清晰標註目前頁數與總頁數，例如：`📄 第 1 / 3 頁 (共 3 頁)`（支援多語系，如英文 `📄 Page 1 / 3 (Total 3 pages)`）。
  - **底部 Footer**：顯示全店總菜單品項數與目前頁數，例如：`📄 目前為第 1 / 3 頁 · 全店共 45 道餐點`。
- **結尾導航按鈕**：
  - 在每頁菜單品項結尾處，系統自動產生可跳轉至其他剩餘頁數的按鈕（例如在第 1 頁時提供 `[ 📄 第 2 頁 ]`、`[ 📄 第 3 頁 ]`；在第 2 頁時提供 `[ 📄 第 1 頁 ]`、`[ 📄 第 3 頁 ]`）。
  - 使用者點擊按鈕，即自動傳送對應分頁指令（如 `週一菜單 第2頁` 或 `菜單 第2頁`），跳出該餐廳對應頁數的餐點品項。

#### 3. 嚴格品項點餐完全隔離防護 (Order Immunity)
- 由於使用者可能直接在聊天室輸入或點擊「`第2頁`」、「`菜單 第2頁`」、「`週一菜單 第2頁`」、「`page 2`」等文字：
  - 系統內建 `isMenuPageCommand` 精準識別分頁指令，並在第一時間優先轉發給菜單分頁處理常式，直接回傳對應頁數菜單。
