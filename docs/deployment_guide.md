...
| `匯入自訂餐廳 [週X] [餐廳名]` | 在群組直接透過試算表自訂工作表名稱匯入菜單 | `匯入自訂餐廳 週三 老王便當` |
| `幫助` | 顯示所有可用指令教學卡片 | `幫助` |
...
1. **Google 試算表選單**：
   ...
   - 系統比對存在後，即自動將該餐廳排程寫入 `WeeklySchedule`，並將菜單填入 `Menu`。若工作表不存在則會跳出提示視窗。
2. **LINE 聊天室文字指令**：
   - 在群組發送指令：`匯入自訂餐廳 [週幾] [餐廳名稱]` 或 `匯入餐廳 [週幾] [餐廳名稱]`。
   - 範例：`匯入自訂餐廳 週三 老王便當`。
   - 機器人將自動檢核工作表並回覆成功訊息與匯入餐點總數。
