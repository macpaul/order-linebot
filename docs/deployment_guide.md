...
| `DEFAULT_LOCALE` | `zh-TW` | **全域系統預設語系**：支援 `zh-TW`、`en`、`ja`、`ko`、`th`、`id`。 |
...
本系統支援繁體中文 (`zh-TW`)、英文 (`en`)、日文 (`ja`)、韓文 (`ko`)、泰文 (`th`) 與印尼文 (`id`) 等 6 國語言。

#### 1. 全域系統預設語系 (`DEFAULT_LOCALE`)
- **設定位置**：試算表 `Config` 頁籤中的 `DEFAULT_LOCALE`。
- **預設值**：`zh-TW`。
- **可選設定**：`zh-TW`、`en`、`ja`、`ko`、`th`、`id`。
...
- **開關效果**：
  ...
  - **`true`（開放使用者自訂）**：
    ...
    - 成員可透過選單或發送「`設定語言`」/「`lang`」呼叫 6 國語言切換卡片。
    - 亦可直接發送語系代碼，如「`設定語言 en`」或「`lang ja`」。
    - 系統會將使用者的語言設定存入 `UserPreferences` 頁籤，此後該成員的個人訂單、收據與幫助均會以其自訂的語言顯示。
    - 支援母語指令直接點餐操作（如英文 `help`、日文 `ヘルプ`、韓文 `도움말`、泰文 `ช่วยเหลือ`、印尼文 `bantuan`）。
