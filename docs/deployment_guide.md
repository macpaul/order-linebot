...
#### 1. 「今日統計 / 統計」無輸出或找不到訂單之成因與修復
- **指令相容擴充**：支援 `今日統計`、`本日統計`、`統計`、`即時統計` 等多種常用輸入法。
- **跨日預約與當日訂餐雙重比對**：
  - 在週梯次預約制度下，成員可能在週一就提早預約了「週二」的便當。此時試算表記錄的下單日期為週一，但梯次為「週二」。
  - 系統在計算週二的「今日統計」時，會同時比對 `dayOfWeek === '週二'` 與 `date === '今日日期'`，確保提早預約與當日下單的餐點均 100% 納入統計。
- **Google Sheets 原生 Date 物件正規化**：
  - Google Sheets 的日期儲存格在 `getValues()` 讀取時為 JavaScript `Date` 物件。若未轉換而直接呼叫字串比對會導致比對失敗。系統已全面加入 `_formatDateValue` 轉換器，統一標準化為 `yyyy-MM-dd`。
- **完整呈現成員應付名冊**：
