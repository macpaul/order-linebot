...
1. **取消選單嚴格隔離（非開單人不顯示他人餐點）**：
   - 於聊天室發送「`取消`」或「`取消餐點`」時，若發送者為一般成員，系統透過三重守衛（資料庫查詢、派發器過濾、Flex 卡片產生器過濾），**選單中 100% 僅呈現該使用者本人的餐點，絕不顯示其他任何成員的餐點**，自然杜絕誤按退訂他人餐點的可能性。
   - 若發送者為開單人，選單則會列出全體成員的有效訂單，且每個品項均標示訂購者暱稱（例如 `👤 [小鮑伯]`），按鈕會帶入對應成員名稱，方便開單人直接一鍵管理。
